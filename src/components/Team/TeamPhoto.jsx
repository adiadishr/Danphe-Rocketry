...
const TeamPhoto = ({ photo, delay, name, position }) => {
    return (
        <div data-aos='zoom-in' data-aos-duration='900' data-aos-delay={delay} className="group rounded-full size-[20rem] overflow-hidden relative">
            ...
            <div className='absolute z-50 flex size-[inherit] items-center justify-center  group-hover:scale-100 opacity-0 group-hover:opacity-100 transition-all duration-[600ms] text-center flex-col px-[5%] gap-2 mt-4'>

                <div className='text-white text-xl'>{name}</div>
                <div className='text-lg text-white font-extralight'>{position}</div>
            </div>
        </div>
    )
}
